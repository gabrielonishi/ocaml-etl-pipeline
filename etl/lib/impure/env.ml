...
(** [get_full_filepath filepath] computes the full file path by combining the current working directory
    with the provided relative file path.

    @param filepath The file path from project root to be combined with the current working directory.
...
(** [fetch_filepaths] retrieves the file paths for orders, order items, and output 
    from environment variables. If a required variable is not set, it raises an exception.

    @return A tuple containing the full file paths for orders, order items, and output.
