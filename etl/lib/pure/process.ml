...
let order_item_in_acc (acc: int list) (order_item: order_item) =
  ...
  if not (int_is_in this_id acc) then 
...
let process_order (order_items : order_item list) :
    order_summary =
  List.fold_left
    (fun acc order_item ->
...
let group_by_ids (order_items : order_item list) : order_summary list =
  ...
  List.fold_left
    (fun (acc: order_summary list) (id: int) ->
      let filtered_order = List.filter (fun (o: order_item) -> o.order_id = id) order_items in
